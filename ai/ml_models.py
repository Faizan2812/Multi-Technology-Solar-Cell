...
def nested_cross_validate(model_class, X, Y, param_grid,
                          n_outer=5, n_inner=3, target_idx=0, seed=42):
    """NESTED cross-validation (v3.0 AI-integrity upgrade).

    Plain k-fold CV with hyperparameters tuned on the SAME folds leaks the
    test information into model selection and produces optimistically biased
    R² [Cawley & Talbot, JMLR 11, 2079 (2010)]. Nested CV separates the two:

        outer loop : unbiased generalization estimate (never used for tuning)
        inner loop : hyperparameter selection on the outer-training data only

    Data-leakage guards enforced here:
      * hyperparameters are chosen per-outer-fold using ONLY that fold's
        training portion;
      * any feature scaling must be fit inside the fold (the from-scratch
        models here are scale-free trees / handle scaling internally);
      * the outer test fold never influences any choice.

    param_grid : list of kwargs dicts to search (e.g. [{'max_depth': 5}, ...]).
    Returns dict with outer-fold metrics (mean/std R², RMSE, MAE), the
    per-fold selected hyperparameters, and — for the audit trail — the
    optimistic non-nested score for comparison.
    """
    rng = np.random.default_rng(seed)
    n = len(X)
    idx = rng.permutation(n)
    fold = n // n_outer
    outer_metrics, chosen = [], []

    for k in range(n_outer):
        te = idx[k * fold:(k + 1) * fold]
        tr = np.concatenate([idx[:k * fold], idx[(k + 1) * fold:]])
        # inner selection on tr only
        best_r2, best_kw = -np.inf, param_grid[0]
        ifold = len(tr) // n_inner
        for kw in param_grid:
            r2s = []
            for j in range(n_inner):
                ite = tr[j * ifold:(j + 1) * ifold]
                itr = np.concatenate([tr[:j * ifold], tr[(j + 1) * ifold:]])
                m = model_class(**kw)
                m.fit(X[itr], Y[itr, target_idx])
                r2s.append(compute_metrics(Y[ite, target_idx],
                                           m.predict(X[ite]))["R2"])
            if np.mean(r2s) > best_r2:
                best_r2, best_kw = np.mean(r2s), kw
        chosen.append(best_kw)
        # unbiased outer evaluation with the selected hyperparameters
        m = model_class(**best_kw)
        m.fit(X[tr], Y[tr, target_idx])
        outer_metrics.append(compute_metrics(Y[te, target_idx], m.predict(X[te])))

    # optimistic (non-nested) comparison score: tune AND test on full CV
    naive_best = -np.inf
    for kw in param_grid:
        avg, _, _ = cross_validate(model_class, X, Y, n_folds=n_outer, **kw)
        naive_best = max(naive_best, avg["R2"])

    keys = outer_metrics[0].keys()
    return {
        "nested_mean": {k: float(np.mean([m[k] for m in outer_metrics])) for k in keys},
        "nested_std": {k: float(np.std([m[k] for m in outer_metrics])) for k in keys},
        "per_fold_hyperparams": chosen,
        "non_nested_best_r2": float(naive_best),
        "optimism_gap_r2": float(naive_best - np.mean([m["R2"] for m in outer_metrics])),
