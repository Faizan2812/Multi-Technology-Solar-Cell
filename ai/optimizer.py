"""
AI / ML Optimization and Surrogate Modeling
===========================================
This module provides optimization and machine-learning utilities built on
top of the physics solver. All components are implemented in pure NumPy.

Components
----------
- Bayesian Optimization with GPR (Matern 5/2 kernel) + Expected Improvement.
- NSGA-II multi-objective optimization (PCE vs stability, etc.).
- Differential Evolution, PSO, Genetic Algorithm (metaheuristic baselines).
- J(V) surrogate MLP with monotonicity regularization (NOT a PINN — see note).
- DeepONet surrogate mapping (device params, voltage) -> current density.
- Permutation-based feature importance (SHAP-like, not true Shapley values).
- Inverse design via DE.
- Active learning (max-uncertainty and EI acquisition).
- Semi-empirical stability lookup (T80).
- Parameter sweeps and material comparisons.
- Rule-based natural-language query parser.

Honest notes on terminology
---------------------------
- The "J(V) surrogate" (class NumpyJVSurrogate, old name NumpyPINN) is a
  feed-forward network with a monotonicity penalty. It does NOT compute
  PDE residuals via autodiff, so it is not a PINN in the Raissi-et-al.
  sense. Legacy imports of NumpyPINN emit a DeprecationWarning.
- The "uncertainty band" from predict_with_uncertainty is a weight-
  perturbation spread, not a Bayesian posterior and not MC-dropout.
- "SHAP" is permutation-based; true Shapley decomposition is not computed.
"""
import numpy as np
...
# ═══════════════════════════════════════════════════════════════════════════════
# J(V) SURROGATE NETWORK (NumPy) with monotonicity regularization
# ═══════════════════════════════════════════════════════════════════════════════
# NOTE: This is NOT a Physics-Informed Neural Network in the Raissi-et-al.
# sense — it does not compute PDE residuals via automatic differentiation.
# It is a small MLP that learns J(V) from sampled data with a soft
# monotonicity penalty (J should decrease with V for a diode). The
# "physics_weight" parameter controls the strength of that penalty.
# For a true PDE-residual PINN you would need autodiff (PyTorch/JAX) and
# a 2-D network taking (x, V) rather than a 1-D V → J mapping.
class NumpyJVSurrogate:
    """Feed-forward J(V) surrogate with monotonicity regularization.
    Customizable architecture. Honest replacement for the old 'NumpyPINN'."""
    ...
    def train(self, X, Y, epochs=300, lr=0.002, physics_weight=0.1):
        """Train on (V, J) data.
        Loss = MSE(Y_pred, Y) + physics_weight · mean(max(dJ/dV, 0)²)
        The second term penalizes non-monotonicity of J(V) — a soft
        shape constraint, NOT a PDE residual."""
        losses = []
    ...
    def predict_with_uncertainty(self, X, n_samples=20, noise_scale=0.01):
        """Weight-perturbation ensemble uncertainty (NOT MC dropout).

        Adds Gaussian noise to the network weights and averages over samples.
        This produces a heuristic pseudo-uncertainty band — it is NOT a
        Bayesian posterior, and it is NOT MC-dropout (no dropout layers are
        used anywhere in this network). For a true Bayesian treatment, use
        an ensemble of independently-trained networks or a framework that
        supports dropout and MC sampling (PyTorch / TensorFlow).
        """
        predictions = []
        for _ in range(n_samples):
            noisy_weights = [w + np.random.randn(*w.shape) * noise_scale for w in self.weights]
            a = X
...
# Backwards-compat alias (DEPRECATED). Existing imports of NumpyPINN keep working.
# New code should use NumpyJVSurrogate and honestly labeled APIs.
class NumpyPINN(NumpyJVSurrogate):
    """DEPRECATED alias for NumpyJVSurrogate.
    This class does NOT compute PDE residuals. The 'PINN' name was inaccurate.
    It is a feed-forward J(V) surrogate with a monotonicity regularizer.
    Kept for backwards compatibility. New code should import NumpyJVSurrogate.
    """

    def __init__(self, *args, **kwargs):
        import warnings
        warnings.warn(
            "NumpyPINN has been renamed to NumpyJVSurrogate. It is NOT a PINN "
            "(it does not compute PDE residuals). See ai/optimizer.py docstring.",
            DeprecationWarning, stacklevel=2)
        super().__init__(*args, **kwargs)


def train_jv_surrogate(voltages, currents, epochs=500, lr=0.002):
    """Train a J(V) surrogate MLP with monotonicity regularization.
    Returns (predicted_J, weight_perturbation_uncertainty, losses, net, v_max, j_scale).

    Note: the 'uncertainty' band returned is a weight-perturbation spread,
    not a Bayesian posterior. See NumpyJVSurrogate.predict_with_uncertainty."""
    jsc = abs(currents[0])
...
def train_pinn_surrogate(voltages, currents, epochs=500, lr=0.002):
    """DEPRECATED alias — use train_jv_surrogate. This function does NOT
    train a PINN (no PDE residual is computed); it trains a feedforward
    J(V) network with monotonicity regularization."""
    import warnings
    warnings.warn(
        "train_pinn_surrogate has been renamed to train_jv_surrogate. "
        "The surrogate is NOT a PINN. See ai/optimizer.py docstring.",
        DeprecationWarning, stacklevel=2)
    return train_jv_surrogate(voltages, currents, epochs=epochs, lr=lr)
...
class DeepONet:
    ...
    def _forward_with_cache(self, x, weights, biases):
        activations = [x]
        pre = []
        a = x
        for i, (w, b) in enumerate(zip(weights, biases)):
    ...
    def train(self, X_params, X_voltage, Y_current, epochs=300, lr=0.005):
        """Train DeepONet with proper backprop through BOTH branch and trunk.

        Shapes:
            X_params  : [F, M]   (F features, M samples)
            X_voltage : [1, K]   (K voltage points)
            Y_current : [M, K]   (current per sample, per voltage)
        Loss: mean-squared error over all (m, k) pairs, sum of branch·trunk.
        """
        # Normalize shapes
        if X_params.ndim == 1:    X_params  = X_params.reshape(-1, 1)
        if X_voltage.ndim == 1:   X_voltage = X_voltage.reshape(1, -1)
        if Y_current.ndim == 1:   Y_current = Y_current.reshape(1, -1)
        F, M = X_params.shape
        _, K = X_voltage.shape
        losses = []
        for epoch in range(epochs):
            # Forward: branch evaluated on all M samples at once → [H, M]
            acts_b, _ = self._forward_with_cache(X_params, self.branch_w, self.branch_b)
            branch_out = acts_b[-1]          # [H, M]
            # Trunk evaluated on all K voltage points → [H, K]
            acts_t, _ = self._forward_with_cache(X_voltage, self.trunk_w, self.trunk_b)
            trunk_out = acts_t[-1]           # [H, K]
            # Prediction: y_hat[m, k] = sum_h branch[h,m] * trunk[h,k] + bias
            Yh = branch_out.T @ trunk_out + self.bias   # [M, K]
            err = Yh - Y_current                           # [M, K]
            loss = float(np.mean(err ** 2))
            losses.append(loss)

            # Gradients on (branch_out, trunk_out, bias)
            # dL/dYh = 2·err / (M*K)
            d_Yh = 2.0 * err / (M * K)
            # dL/dbranch[h,m] = sum_k d_Yh[m,k] * trunk[h,k]
            d_branch_out = trunk_out @ d_Yh.T            # [H, M]
            # dL/dtrunk[h,k]  = sum_m d_Yh[m,k] * branch[h,m]
            d_trunk_out  = branch_out @ d_Yh             # [H, K]
            d_bias       = float(np.sum(d_Yh))

            # Backprop through branch
            grads_w_b = [None]*len(self.branch_w)
            grads_b_b = [None]*len(self.branch_b)
            delta = d_branch_out
            for l in range(len(self.branch_w) - 1, -1, -1):
                a_prev = acts_b[l]
                if l < len(self.branch_w) - 1:
                    delta = delta * self._tanh_deriv(acts_b[l + 1])
                grads_w_b[l] = delta @ a_prev.T
                grads_b_b[l] = np.sum(delta, axis=1)
                delta = self.branch_w[l].T @ delta

            # Backprop through trunk
            grads_w_t = [None]*len(self.trunk_w)
            grads_b_t = [None]*len(self.trunk_b)
            delta = d_trunk_out
            for l in range(len(self.trunk_w) - 1, -1, -1):
                a_prev = acts_t[l]
                if l < len(self.trunk_w) - 1:
                    delta = delta * self._tanh_deriv(acts_t[l + 1])
                grads_w_t[l] = delta @ a_prev.T
                grads_b_t[l] = np.sum(delta, axis=1)
                delta = self.trunk_w[l].T @ delta

            # Update (clip per-parameter gradients to prevent blow-up)
            clip = 1.0
            for l in range(len(self.branch_w)):
                self.branch_w[l] -= lr * np.clip(grads_w_b[l], -clip, clip)
                self.branch_b[l] -= lr * np.clip(grads_b_b[l], -clip, clip)
            for l in range(len(self.trunk_w)):
                self.trunk_w[l]  -= lr * np.clip(grads_w_t[l], -clip, clip)
                self.trunk_b[l]  -= lr * np.clip(grads_b_t[l], -clip, clip)
            self.bias -= lr * d_bias
...
def hybrid_optimize(objective_fn, bounds, budget=150, integer_dims=(),
                    seed=0, explore_frac=0.6, maximize=True):
    """Hybrid mixed-variable optimizer (v3.1, Phase 2).

    Purpose-built for solar-cell design problems that mix CATEGORICAL axes
    (material selection, encoded as integer indices) with continuous ones
    (thicknesses, log-defect density). Strategy:

      Stage 1 (explore, ~60% of budget): differential evolution with
        integer-aware rounding on the categorical dimensions — global
        search across material combinations (Storn & Price 1997).
      Stage 2 (exploit, remaining budget): Nelder-Mead polish of the
        CONTINUOUS dimensions only, with the best categorical choice
        frozen (Nelder & Mead 1965) — local refinement on the smooth
        sub-landscape, where simplex methods excel.

    This addresses the failure mode quantified in utils/opt_benchmark.py:
    quasi-Newton/least-squares methods degrade badly on discontinuous
    categorical axes (finite-difference gradients across material jumps are
    meaningless), while pure global methods waste late-budget evaluations
    re-exploring. The split gives the tool the best of both regimes.

    Returns (best_x, best_val, history).
    """
    import numpy as np
    ...
    def f(x):
        ...
        for d in integer_dims:
            xi[d] = np.floor(xi[d])
        v = objective_fn(xi)
        history["x"].append(xi.copy()); history["val"].append(v)
        history["n"] += 1
        return v

    # ---- Stage 1: integer-aware DE -------------------------------------
    n_explore = int(budget * explore_frac)
    pop_size = max(8, 3 * n_dim)
    P = lo + rng.random((pop_size, n_dim)) * (hi - lo)
    vals = np.array([f(x) for x in P])
    CR = 0.9
    while history["n"] < n_explore:
        for i in range(pop_size):
            if history["n"] >= n_explore:
                break
            F = rng.uniform(0.5, 1.0)          # dithered mutation factor
            a, b, c = P[rng.choice(pop_size, 3, replace=False)]
            mutant = np.clip(a + F * (b - c), lo, hi)
            cross = rng.random(n_dim) < CR
            cross[rng.integers(n_dim)] = True
            trial = np.where(cross, mutant, P[i])
            v = f(trial)
            if sign * v > sign * vals[i]:
                P[i], vals[i] = trial, v
    best_i = int(np.argmax(sign * vals))
    x_best = P[best_i].copy()

    # ---- Stage 2: Nelder-Mead polish on continuous dims -----------------
    # Polish the best (and, budget permitting, the second-best distinct)
    # categorical combination — cheap insurance against DE having ranked
    # two material stacks within noise of each other.
    cont = [d for d in range(n_dim) if d not in integer_dims]
    if cont:
        order = np.argsort(-sign * vals)
        combos, seen = [], set()
        for idx in order:
            key = tuple(int(np.floor(P[idx][d])) for d in integer_dims)
            if key not in seen:
                seen.add(key)
                combos.append(P[idx].copy())
            if len(combos) == (2 if integer_dims else 1):
                break
        for j, x_start in enumerate(combos):
            rem = budget - history["n"]
            share = rem if j == len(combos) - 1 else int(rem * 0.65)
            if share <= 2 * len(cont):
                break
            x_frozen = x_start.copy()
            for d in integer_dims:
                x_frozen[d] = np.floor(x_frozen[d])
            ...
            minimize(f_cont, x_start[cont], method="Nelder-Mead",
                     options={"maxfev": share, "xatol": 1e-3, "fatol": 1e-4})

    vals_arr = np.array(history["val"])
    k = int(np.argmax(sign * vals_arr))
    return history["x"][k], float(vals_arr[k]), history
