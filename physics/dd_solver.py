...
def solve_poisson_newton(mesh: DeviceMesh, psi0, n_in, p_in, bc, T=300.0,
                          ...
                          rho_extra=None, mms_source=None):
    ...
    rho_extra : optional array [cm^-3], additional *signed* fixed charge
        density (e.g. mobile-ion charge, +ve for cations). Enters the Poisson
        RHS as +Q·rho_extra alongside the doping/carrier terms. Used by the
        coupled ion-migration solver (physics/dd_ion.py). Default None = 0.
    mms_source : optional array, additive source term S(x) appended to the
        Poisson residual F_i. Used ONLY by the method-of-manufactured-solutions
        validation (scripts/run_validation.py) so that the *production* Newton
        code path is what gets order-verified. Default None = 0 — has no
        effect on any physical simulation.
    """
    V_T = K_B * T / Q
    N = mesh.N
...
def _thomas(a, b, c, d):
    """Thomas algorithm (tridiagonal solver). Transient over/underflow during
    strongly-screened Picard iterations is tolerated here — every caller
    checks the finiteness/convergence of the final solution."""
    _err = np.seterr(over="ignore", invalid="ignore")
    try:
        return _thomas_core(a, b, c, d)
    finally:
        np.seterr(**_err)


def _thomas_core(a, b, c, d):
    n = len(d)
    cp = np.zeros(n)
    dp = np.zeros(n)
    x = np.zeros(n)
...
def solve_dd(mesh: DeviceMesh, G_profile, V_applied, mat_left, mat_right,
             ...
             warm_start=None, rho_extra=None) -> DDResult:
    """
...
def jv_sweep(mesh, G_profile, mat_left, mat_right,
             ...
             Rs=0.0, Rsh=1e12, rho_extra=None):
