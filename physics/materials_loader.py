...
class Material:
    ...
    sigma_h: float = 1e-15
    Et_offset: float = 0.0
    interface_srv: float = 1e4   # surface recombination velocity at perovskite interface [cm/s]
    refs: str = ""
    category: str = ""
    provenance: Dict = field(default_factory=dict)  # per-parameter provenance
...
def _build_material(name: str, entry: dict, layer_type: str) -> Material:
    ...
    m = Material(
        ...
        interface_srv=g("interface_srv_cm_s", 1e4),
        sigma_e=g("sigma_e_cm2", 1e-15),
        sigma_h=g("sigma_h_cm2", 1e-15),
